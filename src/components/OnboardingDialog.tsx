...
export const OnboardingDialog: React.FC<OnboardingDialogProps> = ({
  ...
  const [pillars, setPillars] = useState<Omit<Pillar, 'id'>[]>([
    { name: 'Health', color: '#4CAF50', order: 0 },
    { name: 'Relationships', color: '#FF69B4', order: 1 },
    { name: 'Work', color: '#FFC107', order: 2 },
  ]);
  ...
  const handleRemovePillar = (index: number) => {
    setPillars(prev => prev.filter((_, i) => i !== index).map((pillar, i) => ({ ...pillar, order: i })));
  };

  const handleUpdatePillar = (index: number, updates: Partial<Omit<Pillar, 'id'>>) => {
    setPillars(prev => prev.map((pillar, i) => i === index ? { ...pillar, ...updates } : pillar));
  };
            ...
            >
              ...
              <div>
                ...
                <Input
                  ...
                  className="bg-balance-surface-elevated border-balance-surface-elevated rounded-balance-sm mt-2 text-center text-lg"
                  autoFocus
                />
              </div>
              ...
              <Button
                ...
                className="w-full bg-health hover:bg-health/90 text-white rounded-balance h-12"
              >
                Continue
            ...
            >
              ...
              <div className="space-y-3">
                {pillars.map((pillar, index) => (
                  <Card key={index} className="surface-elevated p-3">
                    <div className="flex items-center space-x-3">
                      ...
                      <Select
                        value={pillar.color}
                        onValueChange={(color) => handleUpdatePillar(index, { color })}
                      >
              ...
              <div className="flex space-x-3">
                <Button
                  variant="ghost"
                  onClick={() => setStep(1)}
                  className="flex-1 rounded-balance"
                >
                  Back
                </Button>
                <Button
                  onClick={handleComplete}
                  className="flex-1 bg-health hover:bg-health/90 text-white rounded-balance"
                >
                  Get Started
                </Button>
              </div>
